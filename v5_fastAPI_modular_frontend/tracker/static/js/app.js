 ...
 * @param {string} componentName - The name of the component file (e.g., 'auth').
...
async function loadComponent(componentName, targetSelector) {
    try {
        // Passt den Pfad an deine Ordnerstruktur an
...
async function showView(viewName) {
    // Mappt den Ansichtsnamen auf den korrekten Dateinamen
    const component = viewName === 'app' ? 'main' : 'auth';
    await loadComponent(component, '#main-container');
    // WICHTIG: Nach dem Laden der neuen Ansicht müssen die DOM-Referenzen aktualisiert werden.
...
async function logout() {
    ...
    await showView('auth'); // Lädt die Auth-Ansicht
    addAuthEventListeners(); // Fügt die Listener für die Auth-Ansicht hinzu
...
async function initializeApp() {
    ...
    // FÜGT JETZT DIE EVENT LISTENER FÜR DIE APP-VIEW HINZU
    addAppEventListeners();
    
    // NEU: try...catch-Block, um Fehler beim Laden abzufangen
    try {
        await loadAllData();
        await updateDashboard("week");
    } catch (error) {
        // Wenn ein Fehler auftritt (z.B. AuthError), loggen wir den User aus.
        if (error instanceof AuthError) {
            console.error("Authentication failed during data load. Logging out.");
            logout();
        } else {
            // Handle andere Ladefehler hier, falls nötig
...
async function startup() {
    // Lädt zuerst die HTML-Teile, die immer da sind (z.B. Modals)
    await loadComponent('modals', '#modals-container');
    // Initialisiert die Modal-Instanzen im DOM-Objekt, jetzt wo sie geladen sind
    ...
    // Bindet globale Listener, die nicht von Views abhängen
    dom.funMode.toggle.addEventListener('change', handleFunModeToggle);
    
    // Prüft den Login-Status und zeigt die passende Ansicht
