...
// In app.js -> addEventListeners()
    ...
    async initializeApp() {
        ...
        this.auth.navSection.innerHTML = `<button class="btn btn-outline-danger" id="logout-btn">Abmelden</button>`; //den logout button einfügen in die navSection
        document.getElementById("logout-btn").addEventListener("click", () => this.logout()); //eventlistener an logout button adden
    ...
    //!Erklärung loadData
    /*
    bei loadenity data schickt der server uns ein komplettes normalisiertes datenpacket (dank marshmellow) mit allen datensätzen für diesen entity type. 
    dieses array wird dann temporär in der _data variable gespeichert wird und dann mit einer schleich jedes objekt durchläuft.
    da wir das marshmellow schema nutzen sind alle objekte (datensätze) in einer form. 
    der pk wurde für die serialisierung einfach id genannt und die row in unserem table element bekommt die id welche der jeweilige datensatz hat
    was auch die referenz in html ist wodurch wir diese zeile in dem table element ansprechen/finden können. 
    mit dem _columms objekt erstellen wir dynamisch die benötigten lables
    welche auch gleichzeitig als keys dienen um dann die daten aus _data dymaisch für jede row richtig einzuordnen
    */

    async loadAllData() {
        const entities = ["foods", "exercisetypes", "consumptionlogs", "activitylogs"];
        await Promise.all(entities.map(entity => this.loadEntityData(entity)));
    ...
    async handleProfileSubmit(e) {
        ...
        } catch (error) { alert(`Fehler beim Speichern des Profils: ${error.message}`); }
    }
    ...
    openModal(entity, id = null) {
        ...
        form.innerHTML = this.getFormFields(entity);
